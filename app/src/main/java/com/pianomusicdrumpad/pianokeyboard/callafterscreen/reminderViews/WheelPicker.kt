...
class WheelPicker @JvmOverloads constructor(
    ...
    private val SNAP_SCROLL_DURATION = 1500
    ...
    private var mFadingEdgeEnabled = false
    private var mSelectedTextScale = 0.6f
    private var mTypefaceIndex: Int = 3
    ...
    init {
        ...
        mUnSelectedTextColor = attributesArray.getColor(
            R.styleable.WheelPicker_textColor2
            , ContextCompat.getColor(context, R.color.blacktran)
        )
        ...
        mFadingEdgeEnabled = attributesArray.getBoolean(R.styleable.WheelPicker_fadingEdgeEnabled, false)
        ...
        mTextPaint.run {
            ...
            typeface = when(mTypefaceIndex) {
                0 -> Typeface.DEFAULT
                1 -> Typeface.SANS_SERIF
                2 -> Typeface.SERIF
                else -> ResourcesCompat.getFont(context, R.font.popins_semibold) ?: Typeface.DEFAULT
            }
        }
    ...
    private fun computeMaximumWidth(): Int {
        ...
        if (mAdapter != null) {
            return if (!mAdapter!!.getTextWithMaximumLength().isEmpty()) {
                val suggestedWith = mTextPaint.measureText("000000000000000000000").toInt()
    ...
    private fun drawVertical(canvas: Canvas) {
        ...
        while (i < mSelectorItemIndices.size) {
            ...
            if (mSelectorItemValidStatus[i]) {
                ...
                mTextPaint.color = ContextCompat.getColor(context, R.color.blacktran)
            }
    ...
    fun getValue(position: Int): String = when {
        mAdapter != null -> mAdapter!!.getValue(context,position)
    ...
    fun getMaxValue(): String {
        return if (mAdapter != null) {
            mAdapter!!.getValue(context,mMaxIndex)
        } else {
            mMaxIndex.toString()
        }
    }
    ...
    fun getMinValue(): String {
        return if (mAdapter != null) {
            mAdapter!!.getValue(context,mMinIndex)
        } else {
            mMinIndex.toString()
        }
    }
